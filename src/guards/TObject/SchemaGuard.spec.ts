...
describe("SchemaGuard", () => {
  describe("constructor", () => {
    ...
    it("returns an instance of a Guard", () => {
      expect(new SchemaGuard({ name: TString })).toBeInstanceOf(Guard);
    });
  });

  describe("name", () => {
    describe("when a single guard is the schema", () => {
      it("is the name of the guard", () => {
  ...
  describe("isValid", () => {
    describe("when used with a single guard", () => {
      it("returns true, if the guard accepts the value", () => {
        const guard = new SchemaGuard(TString);
        expect(guard.isValid("foo")).toBe(true);
      });

      it("returns false, if the guard rejects the value", () => {
        const guard = new SchemaGuard(TString);
  ...
  describe("cast", () => {
    describe("when the schema is a single guard", () => {
    ...
    describe("when the schema is a guard tree", () => {
      const guard = new SchemaGuard({
        name: TString,
        cart: { peach: TNumber },
      });
